...
void get_ht_results(vector<ht_results> & ht_r, int ht, vector<Cluster *> & clusters){  // LSH_assignment
	int flag;
...
int main( int argc, char ** argv){             
	...
	clusters = pattern3();
	c2 = clock();
	clusters_output(fp , clusters, c2-c1, "I1A2U2" );
	free_Clusters(clusters);
	//algorithm = "1 2 2";
	c1 = clock();
	clusters = pattern4();
	c2 = clock();
	clusters_output(fp , clusters, c2-c1, "I1A1U2" );
	free_Clusters(clusters);
	//algorithm = "1 1 2";
	c1 = clock();
	clusters = pattern5();
